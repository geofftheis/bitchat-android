...
open class EncryptionService(private val context: Context) {
    ...
    private fun setUpEncryptedPrefs() {
        try {
            ...
            //    Both EncryptionService ("bitchat_crypto_secure") and
            //    SecureIdentityStateManager ("bitchat_identity") use
            //    MasterKey.DEFAULT_MASTER_KEY_ALIAS. Deleting the master key
            //    invalidates both, so we must delete both prefs files.
            deletePrefsFiles(SECURE_PREFS_NAME)
            deletePrefsFiles("bitchat_identity")
            ...
            try {
                ...
                prefs = context.getSharedPreferences(SECURE_PREFS_NAME + "_fallback", Context.MODE_PRIVATE)
            }
        }
    }
